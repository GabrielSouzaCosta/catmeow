        ...
        <nav className='bg-white/0.9 font-secondary z-20'>
            <div className='container py-1.5 mx-auto flex justify-between items-center lg:py-4 px-4 z-30 relative'>
                ...
                <div className="lg:hidden" >
                    <Hamburger toggled={isSidebarToggled} onToggle={() => setIsSidebarToggled(prevState => !prevState)} color={isSidebarToggled || context?.theme === 'dark' ? '#E49E77' : '#505050'} />
                </div>
            ...
            <aside className={ ` ${isSidebarToggled ? 'right-0' : '-right-60'} flex lg:hidden fixed inset-y-0 gradient-background dark:bg-darkSecondary transition-all duration-300 w-40 h-screen items-center z-20 drop-shadow-lg rounded-l-sm` }>
                <ul className="flex flex-col gap-y-6 items-center text-darkSecondary dark:text-light text-2xl w-full">
                    <li>
                        <Link href="/cats" className={currRoute === '/cats' ? 'border-b border-primary' : 'hover:text-primary transition-all'}>
                            Cats
                        </Link>
                    </li>
                    <li className={currRoute === '/facts' ? 'border-b border-primary' : 'hover:text-primary transition-all'}>
                        <Link href="/facts">
                            Facts
                        </Link>
                    </li>
                    <li className={currRoute === '/about' ? 'border-b border-primary' : 'hover:text-primary transition-all'}>
                        <Link href="/about">
                            Abouts
                        </Link>
                    </li>
